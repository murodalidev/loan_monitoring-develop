...
def get_target_history(target_id, db_session):
    case_history = []
    target_history = db_session.query(TargetMonitoringHistory).filter(TargetMonitoringHistory.target_monitoring_id == target_id)\
        .order_by(TargetMonitoringHistory.created_at.asc()).all()
