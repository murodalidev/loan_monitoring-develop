...
def accept_or_rework_mib_ended_data(request, db_session):
    info_logger.info("user %s required 'accept_or_rework_mib_ended_data' method", request.from_user)
    mib_ended_data = db_session.query(ProblemsMib).filter(ProblemsMib.id == request.mib_ended_id).first()
    ...
    if request.result_type:
        ...
        get_problems_state_notification.bpi_ended_status = problems_assets_status['проверено']
        
        new_mib_ended_data_history = MibEndedHistory(problems_case_id = request.problems_case_id, 
                                            type_id = CASE_HISTORY.BPI_ENDED.value,
        ...
        set_chain_state_for_bpi_ended(get_problems_case.portfolio.loan_id, db_session)
        ...
        info_logger.info("user %s sent mib_ended_data to rework", request.from_user)
        info_logger.info("user %s sent to rework files: %s", request.from_user, file_path)
        new_problems_monitoring_history = MibEndedHistory(problems_case_id = request.problems_case_id,
                                            type_id = CASE_HISTORY.BPI_ENDED.value,
                                            from_user_id = request.from_user, 
                                            to_user_id= request.to_user,
                                            comment = request.comment,
        ...
        get_problems_state_notification.bpi_ended_status = problems_assets_status['переделать']
        
    mib_ended_data.mib_ended_status_id = status
...
def set_wrong_files(files_id, db_session):
    get_files = db_session.query(ProblemsMibFiles).filter(ProblemsMibFiles.id.in_(files_id)).all()
    file_path = []
