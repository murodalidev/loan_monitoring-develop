...
from app.models.problems_case.auction.auction_history_model import AuctionHistory
...
def accept_or_rework_auction_data(request, db_session):
    info_logger.info("user %s required 'accept_or_rework_auction_data' method", request.from_user)
    auction_data = db_session.query(ProblemsAuction).filter(ProblemsAuction.id == request.auction_id).first()
    ...
    if request.result_type:
        ...
        get_problems_state_notification.auction_status = problems_assets_status['проверено']
        
        new_auction_data_history = JudicialDataHistory(problems_case_id = request.problems_case_id, 
        ...
        db_session.add(new_auction_data_history)
        ...
        file_path = set_wrong_files(request.wrong_files, db_session)
        flush_object(db_session)
        info_logger.info("user %s sent auction_data to rework", request.from_user)
        info_logger.info("user %s sent to rework files: %s", request.from_user, file_path)
        new_problems_monitoring_history = AuctionHistory(problems_case_id = request.problems_case_id,
                                            type_id = CASE_HISTORY.AUCTION.value,
                                            from_user_id = request.from_user, 
                                            to_user_id= request.to_user,
                                            comment = request.comment,
        ...
        get_problems_state_notification.auction_status = problems_assets_status['переделать']
        
    auction_data.auction_status_id = status
...
def set_wrong_files(files_id, db_session):
    get_files = db_session.query(ProblemsAuctionFiles).filter(ProblemsAuctionFiles.id.in_(files_id)).all()
    file_path = []
