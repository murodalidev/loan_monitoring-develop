...
class TargetMonitoringHistory(Base):
    __tablename__='target_monitoring_history'
    ...
    from_user = relationship("Users", foreign_keys="TargetMonitoringHistory.from_user_id")
    to_user_id = Column(Integer, ForeignKey('users.id'), nullable = True, index=True)
    to_user = relationship("Users", foreign_keys="TargetMonitoringHistory.to_user_id")
