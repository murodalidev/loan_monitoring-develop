...
class ScheduledMonitoringHistory(Base):
    __tablename__='scheduled_monitoring_history'
    ...
    from_user = relationship("Users", foreign_keys="ScheduledMonitoringHistory.from_user_id")
    to_user_id = Column(Integer, ForeignKey('users.id'), nullable = True, index=True)
    to_user = relationship("Users", foreign_keys="ScheduledMonitoringHistory.to_user_id")
